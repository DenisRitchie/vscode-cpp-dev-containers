...
{
  uri_builder ub(CasaLensCredentials::events_url + postal_code);

  ub.append_query(CasaLensCredentials::events_keyname, CasaLensCredentials::events_key);
...
{
  utility::string_t weather_url(CasaLensCredentials::weather_url);
...
pplx::task<json::value> CasaLens::get_pictures(const utility::string_t &location, const utility::string_t &count)
{
  ...
  credentials cred(CasaLensCredentials::images_keyname, CasaLensCredentials::images_key);
  config.set_credentials(cred);
  utility::string_t bing_url(CasaLensCredentials::images_url);
...
std::string CasaLens::get_date()
{
  time_t t = time(0);
  struct tm *now;
  std::ostringstream date;

  if (0 == (now = localtime(&t)))
  {
...
{
  uri_builder ub_movie(CasaLensCredentials::movies_url);

  ub_movie.append_query(U("startDate"), get_date());
  ub_movie.append_query(U("zip"), postal_code);
  ub_movie.append_query(CasaLensCredentials::movies_keyname, CasaLensCredentials::movies_key);
  ...
  return tms_client.request(methods::GET)
      ...
      .then([&](json::value movie_result)
            ...
            try
            {
                ...
                auto date = get_date();
                std::string year = date.substr(0, date.find(U("-")));
...
bool CasaLens::is_number(const std::string &s)
{
  return !s.empty() &&
         std::find_if(s.begin(), s.end(), [](char c)
                      { return !std::isdigit(c, std::locale()); }) == s.end();
}
...
void CasaLens::fetch_data(http_request message, const std::string &postal_code, const std::string &location)
{
  ...
  {
    {
      std::scoped_lock<std::mutex> m_rwlock{m_mutex};
    ...
    if (resp_data.is_null())
    {
      ...
      message.reply(status_codes::OK, resp_data).then([](pplx::task<void> t)
                                                      { handle_error(t); });
    }
  }
  catch (...)
...
void CasaLens::get_data(http_request message, const std::string &input_text)
{
  ...
  {
    std::string bing_maps_url(CasaLensCredentials::bmaps_url);
    uri_builder maps_builder;
    maps_builder.append_query(U("locality"), input_text);
    maps_builder.append_query(CasaLensCredentials::bmaps_keyname, CasaLensCredentials::bmaps_key);
        ...
        .then([=](json::value maps_result) mutable
              {
                ...
                ub.append_path(lattitude + U(",") + longitude)
                    .append_query(CasaLensCredentials::bmaps_keyname, CasaLensCredentials::bmaps_key);
  ...
  {
    http_client client(CasaLensCredentials::gmaps_url);
